...
def test_run_martin_iron_ore(iron_ore,subtests):
    ...
    with subtests.test("performance model: drg_taconite_pellets"):
        assert perf_df.loc[perf_df["Name"] == "Pellet Fe Content", "Northshore"].values[0] == approx(67.35, 1e-3)
    with subtests.test("cost model: drg_taconite_pellets"):
        assert cost_df.loc[cost_df["Name"] == "Buildings and other structures","Northshore"].values[0] == approx(1012588246, 1e-3)
    ...
    with subtests.test("performance model: std_taconite_pellets"):
        assert perf_df.loc[perf_df["Name"] == "Pellet Fe Content", "Northshore"].values[0] == approx(65.0, 1e-3)
    with subtests.test("cost model: std_taconite_pellets"):
        assert cost_df.loc[cost_df["Name"] == "Buildings and other structures","Northshore"].values[0] == approx(802586723.3, 1e-3)
    ...
    with subtests.test("performance model: different location"):
        assert perf_df.loc[perf_df["Name"] == "Pellet Fe Content", "United"].values[0] == approx(67.63, 1e-3)
    with subtests.test("cost model: different location"):
        assert cost_df.loc[cost_df["Name"] == "Buildings and other structures","United"].values[0] == approx(1118066189.0, 1e-3)
...
def test_refit_coefficients(iron_ore,subtests):
    ...
    with subtests.test("performance model: refit coefficients"):
        assert perf_df2.loc[perf_df2["Name"] == "Pellet Fe Content", "Northshore"].values[0] == approx(perf_df.loc[perf_df["Name"] == "Pellet Fe Content", "Northshore"].values[0], 1e-3)
    with subtests.test("cost model: refit coefficients"):
        assert cost_df2.loc[cost_df2["Name"] == "Buildings and other structures","Northshore"].values[0] == approx(cost_df.loc[cost_df["Name"] == "Buildings and other structures","Northshore"].values[0], 1e-3)
