...
def load_model(config):
    ...
    if config.performance_model["refit_coeffs"]:
        pd.read_csv(CD / config.performance_model["inputs_fp"], index_col=[0, 1, 2])
...
def iron_win(config):
    ...
    if config.desired_iron_mtpy:
        hydrogen_amount_kgpy = (
            config.desired_iron_mtpy
            * 1000
            * tech_coeffs.loc["Hydrogen"].values[0]
            / config.input_capacity_factor_estimate
        )
        iron_plant_capacity_mtpy = config.desired_iron_mtpy / config.input_capacity_factor_estimate
