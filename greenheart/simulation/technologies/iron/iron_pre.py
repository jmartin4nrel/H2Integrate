...
class Feedstocks:
    # NOTE:
        # I imagine iron_ore_pellet_unitcost and iron_ore_consumption should be named something like raw_ore_X?
    """
    Represents the consumption rates and costs of various feedstocks used in iron ore preprocessing
    production.
    ...
    Attributes:
        ...
        electricity_cost (float):
            Electricity cost per metric tonne of processed ore production ($/metric tonne).
        ...
        iron_ore_consumption (float):
            Iron ore consumption per metric tonne of processed iron ore production (metric tonnes).
        raw_water_consumption (float):
            Raw water consumption per metric tonne of processed iron ore production (metric tonnes).
        lime_consumption (float):
            Lime consumption per metric tonne of processed iron ore production (metric tonnes).
        carbon_consumption (float):
            Carbon consumption per metric tonne of processed iron ore production (metric tonnes).
        hydrogen_consumption (float):
            Hydrogen consumption per metric tonne of processed iron ore production (metric tonnes).
        natural_gas_consumption (float):
            Natural gas consumption per metric tonne of processed iron ore production (GJ-LHV).
        electricity_consumption (float):
            Electricity consumption per metric tonne of processed iron ore production (MWh).
        slag_disposal_unitcost (float):
            Cost per metric tonne of slag disposal ($/metric tonne).
        slag_production (float):
            Slag production per metric tonne of processed iron ore production (metric tonnes).
        maintenance_materials_unitcost (float):
            Cost per metric tonne of annual processed iron ore production at real capacity
            factor ($/metric tonne).
    """
...
@define
class IronPreCostModelConfig:
    """
    Configuration for the iron ore preprocessing cost model, including operational parameters and
    feedstock costs.
    ...
    Attributes:
        ...
        co2_fuel_emissions (float):
            CO2 emissions from fuel per metric tonne of processed iron ore production.
        co2_carbon_emissions (float):
            CO2 emissions from carbon per metric tonne of processed iron ore production.
        surface_water_discharge (float):
            Surface water discharge per metric tonne of processed iron ore production.
    """
...
class IronPreCosts:
    """
    Base dataclass for calculated iron ore preprocessing costs.
    ...
    Note:
        ...
        `run_iron_pre_finance_model`, as well as base data for `IronPreCostModelOutputs`.
    """
...
@define
class IronPreCostModelOutputs(IronPreCosts):
    """
    Outputs of the iron ore preprocessing cost model, extending the IronPreCosts data with total
    cost calculations and specific cost components related to the operation and
    installation of a iron ore preprocessing plant.

    Attributes:
        total_plant_cost (float):
            The total capital expenditure (CapEx) for the iron ore preprocessing plant.
...
@define
class IronPreCapacityModelConfig:
    """
    Configuration inputs for the iron ore preprocessing capacity sizing model, including plant capacity and
    feedstock details.

    Attributes:
        hydrogen_amount_kgpy Optional (float): The amount of hydrogen available in kilograms 
            per year to preprocess iron ore.
        desired_iron_pre_mtpy Optional (float): The amount of desired processed iron ore production in
            metric tonnes per year.
        input_capacity_factor_estimate (float): The estimated iron ore preprocessing plant capacity factor.
    ...
    desired_iron_pre_mtpy: Optional[float] = field(default=None)
    performance_model: Optional[dict] = field(default=None)


    def __attrs_post_init__(self):
        if self.hydrogen_amount_kgpy is None and self.desired_iron_pre_mtpy is None:
            raise ValueError("`hydrogen_amount_kgpy` or `desired_iron_pre_mtpy` is a required input.")

        if self.hydrogen_amount_kgpy and self.desired_iron_pre_mtpy:
            raise ValueError("can only select one input: `hydrogen_amount_kgpy` or `desired_iron_pre_mtpy`.")

@define
class IronPreCapacityModelOutputs:
    """
    Outputs from the iron ore preprocessing size model.

    Attributes:
        iron_pre_plant_size_mtpy (float): If amount of hydrogen in kilograms per year is input, 
            the size of the iron ore preprocessing plant in metric tonnes per year is output.
        hydrogen_amount_kgpy (float): If amount of processed iron ore production in metric tonnes per year is input, 
            the amount of necessary hydrogen feedstock in kilograms per year is output.
    """
    iron_pre_plant_capacity_mtpy: float
    hydrogen_amount_kgpy: float


def run_size_iron_pre_plant_capacity(config: IronPreCapacityModelConfig) -> IronPreCapacityModelOutputs:
    """
    Calculates either the annual processed iron ore in metric tons based on plant capacity and
    available hydrogen or the amount of required hydrogen based on a desired processed iron ore production.

    Args:
        config (IronPreCapacityModelConfig):
    ...
    Returns:
        IronPreCapacityModelOutputs: An object containing iron ore preprocessing plant capacity in metric tons
    ...
    if config.performance_model['name'] == 'placeholder':
        ...
        if config.hydrogen_amount_kgpy:
            iron_pre_plant_capacity_mtpy = (config.hydrogen_amount_kgpy 
                / 1000
        ...
        if config.desired_iron_pre_mtpy:
            hydrogen_amount_kgpy = (config.desired_iron_pre_mtpy 
                * 1000
                * config.feedstocks.hydrogen_consumption
                / config.input_capacity_factor_estimate
            )
            iron_pre_plant_capacity_mtpy = (config.desired_iron_pre_mtpy 
                / config.input_capacity_factor_estimate
            )

        return IronPreCapacityModelOutputs(
            iron_pre_plant_capacity_mtpy=iron_pre_plant_capacity_mtpy,
    ...
    # Otherwise, import model from filepaths - paths either from model_locs or config
        ...
        iron_pre_plant_capacity_mtpy, hydrogen_amount_kgpy = model_outputs
        return IronPreCapacityModelOutputs(
            iron_pre_plant_capacity_mtpy=iron_pre_plant_capacity_mtpy,
            hydrogen_amount_kgpy=hydrogen_amount_kgpy
        )

def run_iron_pre_model(plant_capacity_mtpy: float, plant_capacity_factor: float) -> float:
    """
    Calculates the annual processed iron ore production in metric tons based on plant capacity and
    capacity factor.
    ...
    Returns:
        float: The calculated annual processed iron ore production in metric tons per year.
    """
    iron_pre_production_mtpy = plant_capacity_mtpy * plant_capacity_factor

    return iron_pre_production_mtpy


def run_iron_pre_cost_model(config: IronPreCostModelConfig) -> IronPreCostModelOutputs:
    #TODO: update iron_ore_consumption and iron_ore_pellet_unitcost names based on notes from Feedstocks
    """
    Calculates the capital expenditure (CapEx) and operating expenditure (OpEx) for
    a iron ore prepocessing plant based on the provided configuration.

    Args:
        config (IronPreCostModelConfig):
    ...
    Returns:
        IronPreCostModelOutputs: An object containing detailed breakdowns of capital and
    ...
    if config.cost_model['name'] == 'placeholder':
        ...
        return IronPreCostModelOutputs(
            # CapEx
...
@define
class IronPreFinanceModelConfig:
    """
    Configuration for the iron ore preprocessing finance model, including plant characteristics, financial assumptions, and cost inputs.
    ...
    Attributes:
        ...
        iron_pre_production_mtpy (float): Annual processed iron ore production in metric tons.
        lcoh (float): Levelized cost of hydrogen.
        grid_prices (Dict[str, float]): Electricity prices per unit.
        feedstocks (Feedstocks):
            The feedstocks required for procssed iron ore production, including types and costs.
        costs (Union[IronPreCosts, IronPreCostModelOutputs]):
    ...
    plant_capacity_mtpy: float
    plant_capacity_factor: float
    iron_pre_production_mtpy: float
    lcoh: float
...
@define
class IronPreFinanceModelOutputs:
    """
    Represents the outputs of the iron ore preprocessing finance model, encapsulating the results of financial analysis for processed iron ore production.
    ...
    Attributes:
        ...
        price_breakdown (pd.DataFrame):
            A Pandas DataFrame detailing the cost breakdown for producing processed iron ore,
            including both capital and operating expenses, as well as the impact of
            various cost factors on the overall price of processing iron ore.
    """
...
def run_iron_pre_finance_model(
    # TODO: verify desired key value pair "name":"iron" (iron -> processed iron ore?)
    config: IronPreFinanceModelConfig,
) -> IronPreFinanceModelOutputs:
    """
    Executes the financial model for processed iron ore production, calculating the breakeven price
    of processed iron ore and other financial metrics based on the provided configuration and cost
    models.
    ...
    Returns:
        IronPreFinanceModelOutputs:
            ...
            metrics, summary values, price breakdown, and processed iron ore price breakdown per
            tonne. This output is instrumental in assessing the financial performance
            and breakeven price for the processed iron ore production facility.
    """
    ...
    pf.add_feedstock(
        ...
        usage=1.0,
        unit="Units per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of water per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of lime per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of carbon per metric tonne of processed iron ore",
        cost=feedstocks.carbon_unitcost,
        escalation=config.gen_inflation,
    )
    # NOTE: I assume this should be named Ore or Raw Ore rather than IronPre Ore
    pf.add_feedstock(
        name="Iron Ore",
        usage=feedstocks.iron_ore_consumption,
        unit="metric tonnes of iron ore per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of hydrogen per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="GJ-LHV per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="MWh per metric tonne of processed iron ore",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of slag per metric tonne of processed iron ore",
    ...
    pf.add_coproduct(
        ...
        unit="kg O2 per metric tonne of processed iron ore",
    ...
    if config.save_plots or config.show_plots:
        ...
        pf.plot_capital_expenses(
            fileout=savepaths[0] + "iron_pre_capital_expense_%i.pdf" % (config.design_scenario_id),
            show_plot=config.show_plots,
        )
        pf.plot_cashflow(
            fileout=savepaths[1] + "iron_pre_cash_flow_%i.png"
            % (config.design_scenario_id),
            show_plot=config.show_plots,
        )

        pd.DataFrame.from_dict(data=pf.cash_flow_out).to_csv(
            savepaths[3] + "iron_pre_cash_flow_%i.csv" % (config.design_scenario_id)
        )
    ...
    return IronPreFinanceModelOutputs(
        sol=sol,
        summary=summary,
...
def run_iron_pre_full_model(greenheart_config: dict, save_plots=False, show_plots=False, output_dir="./output/", design_scenario_id=0) -> Tuple[IronPreCapacityModelOutputs, IronPreCostModelOutputs, IronPreFinanceModelOutputs]:
    # TODO: update calls to greenheart_config yaml with appropriate nesting to pull ore config (or another yaml?)
    """
    Runs the full iron ore preprocessing model, including capacity, cost, and finance models.

    Args:
        greenheart_config (dict): The configuration for the greenheart model.

    Returns:
        Tuple[IronPreCapacityModelOutputs, IronPreCostModelOutputs, IronPreFinanceModelOutputs]:
            A tuple containing the outputs of the iron ore preprocessing capacity, cost, and finance models.
    """
    ...
    iron_pre_config = copy.deepcopy(greenheart_config['iron'])

    if iron_pre_config["costs"]["lcoh"] != iron_pre_config["finances"]["lcoh"]:
        raise(ValueError(
            "iron ore preprocessing cost LCOH and iron ore preprocessing finance LCOH are not equal. You must specify both values or neither. \
                If neither is specified, LCOH will be calculated."
            )
        )

    iron_pre_costs = iron_pre_config["costs"]
    iron_pre_capacity = iron_pre_config["capacity"]
    feedstocks = Feedstocks(**iron_pre_costs["feedstocks"])

    # run iron ore preprocessing capacity model to get iron ore preprocessing plant size
    # uses hydrogen amount from electrolyzer physics model
    capacity_config = IronPreCapacityModelConfig(
        feedstocks=feedstocks,
        **iron_pre_capacity
    )
    capacity_config.performance_model = iron_pre_config["performance_model"]
    iron_pre_capacity = run_size_iron_pre_plant_capacity(capacity_config)

    # run iron ore preprocessing cost model
    iron_pre_costs["feedstocks"] = feedstocks
    iron_pre_cost_config = IronPreCostModelConfig(
        plant_capacity_mtpy=iron_pre_capacity.iron_pre_plant_capacity_mtpy,
        **iron_pre_costs
    )
    iron_pre_cost_config.plant_capacity_mtpy = iron_pre_capacity.iron_pre_plant_capacity_mtpy
    iron_pre_cost_config.cost_model = iron_pre_config["cost_model"]
    iron_pre_costs = run_iron_pre_cost_model(iron_pre_cost_config)

    # run iron ore preprocessing finance model
    iron_pre_finance = iron_pre_config["finances"]
    iron_pre_finance["feedstocks"] = feedstocks

    iron_pre_finance_config = IronPreFinanceModelConfig(
        plant_capacity_mtpy=iron_pre_capacity.iron_pre_plant_capacity_mtpy,
        plant_capacity_factor=capacity_config.input_capacity_factor_estimate,
        iron_pre_production_mtpy=run_iron_pre_model(
            iron_pre_capacity.iron_pre_plant_capacity_mtpy,
            capacity_config.input_capacity_factor_estimate,
        ),
        costs=iron_pre_costs,
    ...
    iron_pre_finance = run_iron_pre_finance_model(iron_pre_finance_config)
