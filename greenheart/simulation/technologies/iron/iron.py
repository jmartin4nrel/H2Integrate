...
class Feedstocks:
    """
    Represents the consumption rates and costs of various feedstocks used in iron
    production.
...
class IronCostModelConfig:
    """
    Configuration for the iron cost model, including operational parameters and
    feedstock costs.
...
@define
class IronCostModelOutputs(IronCosts):
    """
    Outputs of the iron cost model, extending the IronCosts data with total
    cost calculations and specific cost components related to the operation and
    installation of a iron production plant.

    Attributes:
        total_plant_cost (float):
            The total capital expenditure (CapEx) for the iron plant.
...
class IronCapacityModelConfig:
    """
    Configuration inputs for the iron capacity sizing model, including plant capacity and
    feedstock details.

    Attributes:
        hydrogen_amount_kgpy Optional (float): The amount of hydrogen available in kilograms 
            per year to make iron.
        desired_iron_mtpy Optional (float): The amount of desired iron production in
            metric tonnes per year.
        input_capacity_factor_estimate (float): The estimated iron plant capacity factor.
    ...
    def __attrs_post_init__(self):
        if self.hydrogen_amount_kgpy is None and self.desired_iron_mtpy is None:
            raise ValueError("`hydrogen_amount_kgpy` or `desired_iron_mtpy` is a required input.")

        if self.hydrogen_amount_kgpy and self.desired_iron_mtpy:
            raise ValueError("can only select one input: `hydrogen_amount_kgpy` or `desired_iron_mtpy`.")

@define
class IronCapacityModelOutputs:
    """
    Outputs from the iron size model.

    Attributes:
        iron_plant_size_mtpy (float): If amount of hydrogen in kilograms per year is input, 
            the size of the iron plant in metric tonnes per year is output.
    ...
    """
    iron_plant_capacity_mtpy: float
    hydrogen_amount_kgpy: float


def run_size_iron_plant_capacity(config: IronCapacityModelConfig) -> IronCapacityModelOutputs:
    """
    ...
    Args:
        config (IronCapacityModelConfig):
    ...
    Returns:
        IronCapacityModelOutputs: An object containing iron plant capacity in metric tons
...
def run_iron_model(plant_capacity_mtpy: float, plant_capacity_factor: float) -> float:
    """
    Calculates the annual iron production in metric tons based on plant capacity and
    capacity factor.
    ...
    Returns:
        float: The calculated annual iron production in metric tons per year.
    """
    iron_production_mtpy = plant_capacity_mtpy * plant_capacity_factor

    return iron_production_mtpy
...
class IronFinanceModelConfig:
    """
    Configuration for the iron finance model, including plant characteristics, financial assumptions, and cost inputs.
    ...
    Attributes:
        ...
        iron_production_mtpy (float): Annual iron production in metric tons.
        lcoh (float): Levelized cost of hydrogen.
        grid_prices (Dict[str, float]): Electricity prices per unit.
        feedstocks (Feedstocks):
            The feedstocks required for iron production, including types and costs.
        costs (Union[IronCosts, IronCostModelOutputs]):
...
def run_iron_full_model(greenheart_config: dict, save_plots=False, show_plots=False, output_dir="./output/", design_scenario_id=0) -> Tuple[IronCapacityModelOutputs, IronCostModelOutputs, IronFinanceModelOutputs]:
    """
    Runs the full iron model, including capacity, cost, and finance models.

    Args:
        greenheart_config (dict): The configuration for the greenheart model.

    Returns:
        Tuple[IronCapacityModelOutputs, IronCostModelOutputs, IronFinanceModelOutputs]:
    ...
    capacity_config = IronCapacityModelConfig(
        feedstocks=feedstocks,
        **iron_capacity
    )
    ...
    iron_finance_config = IronFinanceModelConfig(
        ...
        iron_production_mtpy=run_iron_model(
            iron_capacity.iron_plant_capacity_mtpy,
