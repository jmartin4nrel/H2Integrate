...
def main(config):
    ...
    if config.model['refit_coeffs']:
        ...
        for product in products:
            ...
            keep_rows = np.where(np.logical_not([i in remove_rows for i in \
                                                prod_df.loc[:,'Name'].values]))
            ...
            for key in array_dict:
                ...
                lin_row = np.where(np.logical_and(coeff_df.index.values==key,
                                                    coeff_df.loc[:,'Coeff']=='lin'))[0][0]
                exp_row = np.where(np.logical_and(coeff_df.index.values==key,
                                                    coeff_df.loc[:,'Coeff']=='exp'))[0][0]
    ...
    for item_name in prod_coeffs[prod_coeffs["Type"] == "capital"]["Name"].unique():
        ...
        item_data = prod_coeffs[(prod_coeffs["Name"] == item_name) & (prod_coeffs["Type"] == "capital")]
    ...
    cost = (365
        * (prod_coeffs.loc[prod_coeffs["Name"] == "% Skilled Labor", product].values[0]/100 * 
            np.mean(top_down_coeffs["Skilled Labor Cost"]['values'][td_start_idx:td_end_idx])
        + prod_coeffs.loc[prod_coeffs["Name"] == "% Unskilled Labor", product].values[0]/100 * 
    ...
    cost = prod_coeffs.loc[prod_coeffs["Name"] == "Maintenance Labor Cost", product].values[0]  * total_plant_cost
    labor_cost_maintenance = cost
    fixed_costs['labor_cost_annual_operation'] = cost

    cost = prod_coeffs.loc[prod_coeffs["Name"] == "Administrative & Support Labor Cost", product].values[0] * (
        labor_cost_annual_operation + labor_cost_maintenance
    )
    labor_cost_admin_support = cost
    fixed_costs['labor_cost_admin_support'] = cost

    cost = prod_coeffs.loc[prod_coeffs["Name"] == "Property Tax & Insurance", product].values[0] * total_plant_cost
    ...
    non_fuel_consumables_onemonth = (
        ...
        * (
            ...
            + perf_ds['Reformer Catalyst'] * top_down_coeffs["Reformer Catalyst"]['values'][td_start_idx]
        )
        / 12
    )
    ...
    preproduction_cost = prod_coeffs.loc[prod_coeffs["Name"] == "Preproduction", product].values[0] * total_plant_cost

    fuel_consumables_60day_supply_cost = non_fuel_consumables_onemonth * 12 / 365 * 60

    spare_parts_cost = prod_coeffs.loc[prod_coeffs["Name"] == "Spare Parts", product].values[0] * total_plant_cost
    land_cost = prod_coeffs.loc[prod_coeffs["Name"] == "Land", product].values[0] * plant_capacity_mtpy
    misc_owners_costs = prod_coeffs.loc[prod_coeffs["Name"] == "Other Owners's Costs", product].values[0] * total_plant_cost
    ...
    new_cost = [product,"Installation cost",'other',str(model_year)+" $ per year",installation_cost]
    costs_df.loc[len(costs_df)] = new_cost
