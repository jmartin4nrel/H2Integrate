...
class Feedstocks:
    """
    Represents the consumption rates and costs of various feedstocks used in iron post-processing.
    ...
    Attributes:
        ...
        electricity_cost (float):
            Electricity cost per metric tonne of post-processed iron production ($/metric tonne).
        ...
        raw_water_consumption (float):
            Raw water consumption per metric tonne of post-processed iron production (metric tonnes).
        lime_consumption (float):
            Lime consumption per metric tonne of post-processed iron production (metric tonnes).
        carbon_consumption (float):
            Carbon consumption per metric tonne of post-processed iron production (metric tonnes).
        hydrogen_consumption (float):
            Hydrogen consumption per metric tonne of post-processed iron production (metric tonnes).
        natural_gas_consumption (float):
            Natural gas consumption per metric tonne of post-processed iron production (GJ-LHV).
        electricity_consumption (float):
            Electricity consumption per metric tonne of post-processed iron production (MWh).
        slag_disposal_unitcost (float):
            Cost per metric tonne of slag disposal ($/metric tonne).
        slag_production (float):
            Slag production per metric tonne of post-processed iron production (metric tonnes).
        maintenance_materials_unitcost (float):
            Cost per metric tonne of annual post-processed iron slab production at real capacity
            factor ($/metric tonne).
    """
...
@define
class IronPostCostModelConfig:
    """
    Configuration for the iron post-processing cost model, including operational parameters and
    feedstock costs.
    ...
    Attributes:
        ...
        co2_fuel_emissions (float):
            CO2 emissions from fuel per metric tonne of post-processed iron production.
        co2_carbon_emissions (float):
            CO2 emissions from carbon per metric tonne of post-processed iron production.
        surface_water_discharge (float):
            Surface water discharge per metric tonne of post-processed iron production.
    """
...
class IronPostCosts:
    """
    Base dataclass for calculated iron post-processing costs.
    ...
    Note:
        ...
        `run_iron_post_finance_model`, as well as base data for `IronPostCostModelOutputs`.
    """
...
@define
class IronPostCostModelOutputs(IronPostCosts):
    """
    Outputs of the iron post-processing cost model, extending the IronPostCosts data with total
    cost calculations and specific cost components related to the operation and
    installation of a iron post-processing plant.

    Attributes:
        total_plant_cost (float):
            The total capital expenditure (CapEx) for the iron post-processing plant.
...
@define
class IronPostCapacityModelConfig:
    """
    Configuration inputs for the iron post-processing capacity sizing model, including plant capacity and
    feedstock details.

    Attributes:
        hydrogen_amount_kgpy Optional (float): The amount of hydrogen available in kilograms 
            per year to make iron.
        desired_iron_post_mtpy Optional (float): The amount of desired post-processed iron production in
            metric tonnes per year.
        input_capacity_factor_estimate (float): The estimated iron post-processing plant capacity factor.
    ...
    desired_iron_post_mtpy: Optional[float] = field(default=None)
    performance_model: Optional[dict] = field(default=None)


    def __attrs_post_init__(self):
        if self.hydrogen_amount_kgpy is None and self.desired_iron_post_mtpy is None:
            raise ValueError("`hydrogen_amount_kgpy` or `desired_iron_post_mtpy` is a required input.")

        if self.hydrogen_amount_kgpy and self.desired_iron_post_mtpy:
            raise ValueError("can only select one input: `hydrogen_amount_kgpy` or `desired_iron_post_mtpy`.")

@define
class IronPostCapacityModelOutputs:
    """
    Outputs from the iron post-processing size model.

    Attributes:
        iron_post_plant_size_mtpy (float): If amount of hydrogen in kilograms per year is input, 
            the size of the iron post-processing plant in metric tonnes per year is output.
        hydrogen_amount_kgpy (float): If amount of post-processed iron production in metric tonnes per year is input, 
            the amount of necessary hydrogen feedstock in kilograms per year is output.
    """
    iron_post_plant_capacity_mtpy: float
    hydrogen_amount_kgpy: float


def run_size_iron_post_plant_capacity(config: IronPostCapacityModelConfig) -> IronPostCapacityModelOutputs:
    """
    Calculates either the annual post-processed iron production in metric tons based on plant capacity and
    available hydrogen or the amount of required hydrogen based on a desired post-processed iron production.

    Args:
        config (IronPostCapacityModelConfig):
    ...
    Returns:
        IronPostCapacityModelOutputs: An object containing iron post-processing plant capacity in metric tons
    ...
    if config.performance_model['name'] == 'placeholder':
        ...
        if config.hydrogen_amount_kgpy:
            iron_post_plant_capacity_mtpy = (config.hydrogen_amount_kgpy 
                / 1000
        ...
        if config.desired_iron_post_mtpy:
            hydrogen_amount_kgpy = (config.desired_iron_post_mtpy 
                * 1000
                * config.feedstocks.hydrogen_consumption
                / config.input_capacity_factor_estimate
            )
            iron_post_plant_capacity_mtpy = (config.desired_iron_post_mtpy 
                / config.input_capacity_factor_estimate
            )

        return IronPostCapacityModelOutputs(
            iron_post_plant_capacity_mtpy=iron_post_plant_capacity_mtpy,
    ...
    # Otherwise, import model from filepaths - paths either from model_locs or config
        ...
        iron_post_plant_capacity_mtpy, hydrogen_amount_kgpy = model_outputs
        return IronPostCapacityModelOutputs(
            iron_post_plant_capacity_mtpy=iron_post_plant_capacity_mtpy,
            hydrogen_amount_kgpy=hydrogen_amount_kgpy
        )

def run_iron_post_model(plant_capacity_mtpy: float, plant_capacity_factor: float) -> float:
    """
    Calculates the annual post-processed iron production in metric tons based on plant capacity and
    capacity factor.
    ...
    Returns:
        float: The calculated annual post-processed iron production in metric tons per year.
    """
    iron_post_production_mtpy = plant_capacity_mtpy * plant_capacity_factor

    return iron_post_production_mtpy


def run_iron_post_cost_model(config: IronPostCostModelConfig) -> IronPostCostModelOutputs:
    """
    Calculates the capital expenditure (CapEx) and operating expenditure (OpEx) for
    a iron post-processing plant based on the provided configuration.

    Args:
        config (IronPostCostModelConfig):
    ...
    Returns:
        IronPostCostModelOutputs: An object containing detailed breakdowns of capital and
    ...
    if config.cost_model['name'] == 'placeholder':
        ...
        return IronPostCostModelOutputs(
            # CapEx
...
@define
class IronPostFinanceModelConfig:
    """
    Configuration for the iron post-processing finance model, including plant characteristics, financial assumptions, and cost inputs.
    ...
    Attributes:
        ...
        iron_post_production_mtpy (float): Annual post-processed iron production in metric tons.
        lcoh (float): Levelized cost of hydrogen.
        grid_prices (Dict[str, float]): Electricity prices per unit.
        feedstocks (Feedstocks):
            The feedstocks required for post-processed iron production, including types and costs.
        costs (Union[IronPostCosts, IronPostCostModelOutputs]):
    ...
    plant_capacity_mtpy: float
    plant_capacity_factor: float
    iron_post_production_mtpy: float
    lcoh: float
...
@define
class IronPostFinanceModelOutputs:
    """
    Represents the outputs of the iron post-processing finance model, encapsulating the results of financial analysis for post-processed iron production.
    ...
    Attributes:
        ...
        price_breakdown (pd.DataFrame):
            A Pandas DataFrame detailing the cost breakdown for post-processing iron,
...
def run_iron_post_finance_model(
    # TODO: verify desired key value pair "name":"iron" (iron -> iron post-proecessing?)
    config: IronPostFinanceModelConfig,
) -> IronPostFinanceModelOutputs:
    """
    Executes the financial model for iron post-processing, calculating the breakeven price
    of post-processed iron and other financial metrics based on the provided configuration and cost
    models.
    ...
    Returns:
        IronPostFinanceModelOutputs:
            ...
            metrics, summary values, price breakdown, and post-processed iron price breakdown per
            tonne. This output is instrumental in assessing the financial performance
            and breakeven price for the iron post-processing facility.
    """
    ...
    pf.add_feedstock(
        ...
        usage=1.0,
        unit="Units per metric tonne of post-processed iron",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of water per metric tonne of post-processed iron",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of lime per metric tonne of post-processed iron",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of carbon per metric tonne of post-processed iron",
    ...
    pf.add_feedstock(
        ...
        unit="GJ-LHV per metric tonne of post-processed iron",
    ...
    pf.add_feedstock(
        ...
        unit="MWh per metric tonne of post-processed iron",
    ...
    pf.add_feedstock(
        ...
        unit="metric tonnes of slag per metric tonne of post-processed iron",
    ...
    pf.add_coproduct(
        ...
        unit="kg O2 per metric tonne of post-processed iron",
    ...
    if config.save_plots or config.show_plots:
        ...
        pf.plot_capital_expenses(
            fileout=savepaths[0] + "iron_post_capital_expense_%i.pdf" % (config.design_scenario_id),
            show_plot=config.show_plots,
        )
        pf.plot_cashflow(
            fileout=savepaths[1] + "iron_post_cash_flow_%i.png"
            % (config.design_scenario_id),
            show_plot=config.show_plots,
        )

        pd.DataFrame.from_dict(data=pf.cash_flow_out).to_csv(
            savepaths[3] + "iron_post_cash_flow_%i.csv" % (config.design_scenario_id)
        )
    ...
    return IronPostFinanceModelOutputs(
        sol=sol,
        summary=summary,
...
def run_iron_post_full_model(greenheart_config: dict, save_plots=False, show_plots=False, output_dir="./output/", design_scenario_id=0) -> Tuple[IronPostCapacityModelOutputs, IronPostCostModelOutputs, IronPostFinanceModelOutputs]:
    # TODO: update calls to greenheart_config yaml with appropriate nesting to pull ore config (or another yaml?)
    """
    Runs the full iron post-processing model, including capacity, cost, and finance models.

    Args:
        greenheart_config (dict): The configuration for the greenheart model.

    Returns:
        Tuple[IronPostCapacityModelOutputs, IronPostCostModelOutputs, IronPostFinanceModelOutputs]:
            A tuple containing the outputs of the iron post-processing capacity, cost, and finance models.
    """
    ...
    iron_post_config = copy.deepcopy(greenheart_config['iron'])

    if iron_post_config["costs"]["lcoh"] != iron_post_config["finances"]["lcoh"]:
        raise(ValueError(
            "iron post-processing cost LCOH and iron post-processing finance LCOH are not equal. You must specify both values or neither. \
                If neither is specified, LCOH will be calculated."
            )
        )

    iron_post_costs = iron_post_config["costs"]
    iron_post_capacity = iron_post_config["capacity"]
    feedstocks = Feedstocks(**iron_post_costs["feedstocks"])

    # run iron post-processing capacity model to get iron post-processing plant size
    # uses hydrogen amount from electrolyzer physics model
    capacity_config = IronPostCapacityModelConfig(
        feedstocks=feedstocks,
        **iron_post_capacity
    )
    capacity_config.performance_model = iron_post_config["performance_model"]
    iron_post_capacity = run_size_iron_post_plant_capacity(capacity_config)

    # run iron post-processing cost model
    iron_post_costs["feedstocks"] = feedstocks
    iron_post_cost_config = IronPostCostModelConfig(
        plant_capacity_mtpy=iron_post_capacity.iron_post_plant_capacity_mtpy,
        **iron_post_costs
    )
    iron_post_cost_config.plant_capacity_mtpy = iron_post_capacity.iron_post_plant_capacity_mtpy
    iron_post_cost_config.cost_model = iron_post_config["cost_model"]
    iron_post_costs = run_iron_post_cost_model(iron_post_cost_config)

    # run iron post-processing finance model
    iron_post_finance = iron_post_config["finances"]
    iron_post_finance["feedstocks"] = feedstocks

    iron_post_finance_config = IronPostFinanceModelConfig(
        plant_capacity_mtpy=iron_post_capacity.iron_post_plant_capacity_mtpy,
        plant_capacity_factor=capacity_config.input_capacity_factor_estimate,
        iron_post_production_mtpy=run_iron_post_model(
            iron_post_capacity.iron_post_plant_capacity_mtpy,
            capacity_config.input_capacity_factor_estimate,
        ),
        costs=iron_post_costs,
    ...
    iron_post_finance = run_iron_post_finance_model(iron_post_finance_config)
